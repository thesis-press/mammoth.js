...
function DocumentConversion(options, comments) {
    ...
    function convertRun(run, messages, options) {
        ...
        if (run.isUnderline) {
            paths.push(htmlPaths.element("u", {}, { fresh: false }));
        }
        ...
        if (run.fontSize) {
            paths.push(
                htmlPaths.element(
                    "span",
                    {
                        "font-size": run.fontSize.toString() + "pt",
                    },
                    { fresh: false }
                )
            );
        ...
        if (run.spacing) {
            paths.push(
                htmlPaths.element(
                    "span",
                    {
                        "line-spacing": run.spacing,
                    },
                    { fresh: false }
                )
            );
        }
        if (run.beforeSpacing) {
            paths.push(
                htmlPaths.element(
                    "span",
                    {
                        "before-spacing": run.beforeSpacing,
                    },
                    { fresh: false }
                )
            );
        ...
        if (run.afterSpacing) {
            paths.push(
                htmlPaths.element(
                    "span",
                    {
                        "after-spacing": run.afterSpacing,
                    },
                    { fresh: false }
                )
            );
        ...
        var body = convertElements(comment.body, messages, options).concat([
            Html.nonFreshElement("p", {}, [
                ...
                Html.freshElement(
                    ...
                    {
                        href:
                            "#" + referenceHtmlId("comment", comment.commentId),
                    },
                    [Html.text("↑")]
                ),
            ]),
