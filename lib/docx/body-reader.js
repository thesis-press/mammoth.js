...
function BodyReader(options) {
    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function (style) {
            ...
            return {
                ...
                isBold: element.first("w:b"),
                isUnderline: readUnderline(element.first("w:u")),
                isItalic: readBooleanElement(element.first("w:i")),
                isStrikethrough: readBooleanElement(element.first("w:strike")),
                isAllCaps: readBooleanElement(element.first("w:caps")),
                isSmallCaps: readBooleanElement(element.first("w:smallCaps")),
                indent: element.firstOrEmpty("w:ind").attributes["w:left"],
            };
        });
    }
    ...
    function readUnderline(element) {
        if (element) {
            ...
            return (
                value && value !== "false" && value !== "0" && value !== "none"
            );
        } else {
            return false;
        }
    ...
    function readStyle(element, styleTagName, styleType, findStyleById) {
        ...
        if (styleElement) {
            ...
            if (styleId) {
                ...
                if (style && style.runStyle) {
                    runStyle = style.runStyle;
                }
            }
        }
        return elementResultWithMessages(
            { styleId: styleId, name: name, runStyle },
            messages
        );
    }
    ...
    var xmlElementReaders = {
        ...
        "w:pPr": function (element) {
            return readParagraphStyle(element).map(function (style) {
                let border = element.first("w:pBdr");
                const numId = element.first("w:numPr")?.first("w:numId")
                    ?.attributes["w:val"];
                ...
                if (border) {
                    borderElements.forEach((borderElement) => {
                        const val =
                            border.first(borderElement)?.attributes["w:val"];
                        if (val && val !== "nil") {
                            hasBorder = true;
                        }
                    });
                ...
                return {
                    ...
                    runStyle: {
                        ...
                        ...style.runStyle,
                        indent: readParagraphIndent(
                            element.firstOrEmpty("w:ind")
                        ),
                    },
                    alignment: element.firstOrEmpty("w:jc").attributes["w:val"],
                    numbering: {
                        ...readNumberingProperties(
                            style.styleId,
                        ...
                        ...(style.runStyle?.numId && {
                            numId: style.runStyle.numId,
                            fromStyle: true,
                        }),
                        ...(numId && { numId, fromStyle: false }),
                    },
    ...
    function readImage(imageFile, altText, params = {}) {
...
function readNumberingProperties(styleId, element, numbering, numId, numLevel) {
    let numValues;
    if (numId) {
        numValues = numbering.findLevel(numId, numLevel || "0");
    }
    if (styleId != null) {
        var levelByStyleId = numbering.findLevelByParagraphStyleId(styleId);
        if (levelByStyleId != null) {
            return { ...numValues, ...levelByStyleId };
        }
    }
